...
pub contract PioneerNFT:NonFungibleToken{
   ...
   pub struct PriceMeta {
    ...
    //     //上架
    //     pub fun addSaleList(tokenID:UInt64,price:UFix64){
    //         PioneerNFT.prices[tokenID] = price
    //         emit ForSale(id: tokenID, price: price, owner: self.owner?.address)
    //     }
    //     //下架
    //     pub fun cancelSale(tokenID:UInt64){
    //         PioneerNFT.prices.remove(key:tokenID)
    //         PioneerNFT.prices[tokenID] = nil
    //     }
    //     //通过id 获取价格
    //     pub fun idPrice(tokenID:UInt64):UFix64? {
    //         return PioneerNFT.prices[tokenID]
    //     }
        
    //     //返回在售nftlist
    //     pub fun getIDs():[UInt64] {
    //         return PioneerNFT.prices.keys
    //     }
    ...
    // }
          //上架
    pub fun addSaleList(id:UInt64,url:String,price:UFix64,name: String,description: String,officialUrl: String,creator: Address,properties: {String:String}?){
        let priceMeta = PriceMeta(price:price,url:url,name:name,description:description,officialUrl:officialUrl,creator:creator,properties:properties)
        PioneerNFT.pricesMapping[id] = priceMeta
    }
    //下架
    pub fun cancelSale(tokenID:UInt64){
        PioneerNFT.pricesMapping.remove(key:tokenID)
        PioneerNFT.pricesMapping[tokenID] = nil
     }

    pub fun getIDs():{UInt64: PriceMeta} {
        return PioneerNFT.pricesMapping
     }
